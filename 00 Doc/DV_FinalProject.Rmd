...
The dataframe below displays a left join between the state employment data and the state housing data joined on state. There are also mutated columns added that represent calculated values based on existing columns, such as Average Employment (AVG_EMP).
```{r, eval=TRUE, echo=TRUE}
...
This dataframe contains the same combination of Top Companies data as the one above,except it is filtered to only show results for Texas companies and industries.
```{r, eval=TRUE, echo=TRUE}
data.frame(head(dfTopCompanies3))
```
**************

This dataframe is a left join between a dataframe of the Top Companies data, grouped by state, with dfSTATEHOUSINGTOTALS from above. This displays a wide range of industry, employment, and housing data for each state. Included are calculated columns displaying employment rate, ratio of water to total area, ratio of land to total area, average employees per establishment, average pay (in thousands of dollars), and average growth and revenue.
```{r, eval=TRUE, echo=TRUE}
