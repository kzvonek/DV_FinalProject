...
viz3 <- melt(dfNormalized) %>% ggplot(aes(fill=variable, alpha=0.01)) + 
  ...
  scale_x_continuous(name="Normalized Standard Deviation", labels = comma) + scale_fill_discrete(name="Measure", labels=c("Employment Rate", "Average Employment", "Averge Annual Pay", "Average Growth"))
